...
public class ConfigurationValidator {
    ...
    public func validAuthentication(node: Node) -> Bool {
        return node.authentication != nil
    }

    public func validAdministratorPassword(node: Node) -> Bool {
        do {
    ...
    private func validateConnections() throws {
        ...
        do {
            ...
            throw Error("Invalid credentials for connection. Configuration file needs to be updated! Please run `\(SharedConstants.configuration) \(SharedConstants.authentication)` command. Got \(error)".red)
        }
    }
    ...
    private func validateAuthentication() throws {
        guard configuration.nodes.allSatisfy({ validAuthentication(node: $0) }) else {
            throw Error("Invalid credentials found. Configuration file needs to be updated! Please run `\(SharedConstants.configuration) \(SharedConstants.authentication)` command".red)
        }
    }
    ...
    private func validateAdministratorPassword() throws {
        guard configuration.nodes.allSatisfy({ validAdministratorPassword(node: $0) }) else {
            throw Error("Invalid administrator password found. Configuration file needs to be updated! Please run `\(SharedConstants.configuration) \(SharedConstants.authentication)` command".red)
        }
    }
}
