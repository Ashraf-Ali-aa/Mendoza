...
class PluginInitCommand: Command {
    ...
    let acceptFlag = Flag(short: nil, long: "accept", help: "Accept command")
    ...
    func run() -> Bool {
        do {
            let initer = PluginInit(configurationUrl: configuration.value!, name: pluginName.value!, accept: acceptFlag.value)
            try initer.run()
        } catch {
